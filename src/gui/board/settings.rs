...
fn render_language_selector(board: &mut RopyBoard, cx: &mut Context<RopyBoard>) -> impl IntoElement {
...
fn render_theme_selector(board: &mut RopyBoard, cx: &mut Context<RopyBoard>) -> impl IntoElement {
    ...
    h_flex()
        ...
        .children(theme_names.into_iter().enumerate().map(|(index, name)| {
            ...
            let mut button = Button::new(("theme-button", index))
                .small()
                .label(name);
            ...
            h_flex()
                ...
                .child({
                    ...
                    button = if board.autostart_enabled {
                        button.primary().label(board.i18n.t("settings_autostart_on"))
                    } else {
